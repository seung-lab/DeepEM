...
def get_augmentation(is_train, box=None, interp=False, missing=7, blur=7,
    ...
    to_blend.append(MisalignPlusMissing((3,15), random=is_train))
    if missing > 0:
        to_blend.append(MixedMissingSection(
                maxsec=missing, individual=False, random=is_train))
    if lost:
        to_blend.append(Blend([
            Compose([LostSection(1), LostSection(1)]),
            LostPlusMissing(random=is_train)
        ]))
